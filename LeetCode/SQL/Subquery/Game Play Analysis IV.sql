...
--Step1: finding the minimum event date for each player in a CTE
...
--Step2: Join CTE and Activity table based on player id and date difference. 
-- if date difference between event date and minimum event date (for each player ID) is one day then 
-- count the number of players and divide it by total number of players

SELECT ROUND(COUNT(a.player_id)*1.0/(SELECT COUNT(DISTINCT player_id) FROM ACtivity),2) fraction  
FROM Activity a, MinDate md
WHERE DATEDIFF(DAY, md.MinDate, event_date) = 1 AND a.player_id = md.player_id
