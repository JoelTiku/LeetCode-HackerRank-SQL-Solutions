...
(
    SELECT 
        s.student_id,
        s.student_name,
        CASE WHEN MIN(e.score) OVER(PARTITION BY e.exam_id) = e.score THEN 1 
			 ELSE 0 
		END AS is_min_score,

		CASE WHEN MAX(e.score) OVER(PARTITION BY e.exam_id) = e.score THEN 1 
		     ELSE 0 
		END AS is_max_score
    FROM
        Exam e
    INNER JOIN
		...
		/*
		+------------+--------------+------------------+--------------+
		| student_id | student_name | is_min_score     | is_max_score |
		+------------+--------------+------------------+--------------+
		| 1          |  Daniel      |    1			   |      0       |
		| 2          |  Jade        |    0			   |      0	      |
		| 3          |  Stella      |    0			   |      1	      |
